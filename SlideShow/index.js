...
import isMobileView from "js/ui/isMobileView";
...
export default class SlideShow {
  ...
  constructor(settings) {
    const { id, label, hasPicker = true, hasNavArrows = true, hasEqualSlideHeight = true, animationStyle = "slide" } = settings;
    this.id = id;
    this.label = label;
  ...
  validateSlides() {
    ...
    if (!["slide", "fade", "none"].includes(this.animationStyle)) {
      console.warn(`Invalid animation style "${this.animationStyle}" for Slide Show. Must be one of the following: ${this.animationStyle.join(", ")}`);
      return false;
    }
